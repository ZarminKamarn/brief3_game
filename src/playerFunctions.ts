...
export function turnCharacter(dir: string): void {
  ...
  const curTile: HTMLElement | null = document.getElementById(
    generateId(curCoord[0], curCoord[1])
  );
  ...
  if (curTile) {
    ...
    generateGridImage(curCoord[0], curCoord[1]);
  }
}
...
export function canCharacterMove(
  posX: number,
  posY: number,
  dir: string
): boolean {
...
export function shoot(): void {
  if (!shotAmmo && variables.ammo > 0) {
    ...
    const curCoord: Array<number> = getCurrentCoord();
    const coordShoot: Array<number> = getCoordAfterMove(
      curCoord[0],
      curCoord[1],
      dir
    );
    ...
    if (canBlocMove(coordShoot[0], coordShoot[1], dir)) {
      ...
      generateGridImage(coordShoot[0], coordShoot[1]);
...
export async function drawShootMovement(
  posX: number,
  posY: number,
  dir: string
) {
