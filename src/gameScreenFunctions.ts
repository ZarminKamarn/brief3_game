...
export function drawEndGameScreen(div: HTMLElement): void {
  removeGameListener();
  const divEndScreen: HTMLDivElement = generateEndGameDiv(
...
export function drawEndLvlScreen(div: HTMLElement): void {
  removeGameListener();
  const divEndScreen: HTMLDivElement = generateEndGameDiv(
    `Vous avez fini le niveau ${variables.lvlNumber}!`
  );
...
export function drawGameOverScreen(div: HTMLElement): void {
  removeGameListener();
  const divEndScreen: HTMLDivElement = generateEndGameDiv(
    "Game Over! Vous n'avez plus de vie."
  );
...
function generateEndGameDiv(txt: string): HTMLDivElement {
  ...
  divEndScreen.appendChild(divText);
  return divEndScreen;
}
...
export function generateGridImage(posX: number, posY: number): void {
  const tile: HTMLElement | null = document.getElementById(
    generateId(posX, posY)
  );
  if (tile) {
    resetDiv(tile);
    const src: string = correspondance[variables.curLvl[posX][posY]];
    if (src !== "") {
...
export function addGameListener(): void {
  if (arrowUp && arrowLeft && arrowDown && arrowRight) {
    const arrArrows: Array<HTMLElement> = [
      arrowUp,
      arrowDown,
      arrowRight,
      arrowLeft,
...
export function arrowPressedEvent(event: MouseEvent): void {
  if (gameScreen) {
    // event.target me renvoie un EventTarget correspondant a l'image
    // donc je récupère son parent : la div pour savoir quel bouton a ete appuye
